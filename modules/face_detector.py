...
class FaceDetector:
    ...
    def detect_and_crop(
        ...
        if self._deepface_ready:
            try:
                ...
                if objs and len(objs) > 0:
                    ...
                    emb = obj["embedding"]
                    fa = obj["facial_area"]
                    region = {"x": fa["x"], "y": fa["y"], "w": fa["w"], "h": fa["h"]}
        ...
        if region is None:
            ...
            console.log(
                "[yellow]Using centre-crop fallback — no face model available[/]"
            )
    ...
    def compare_faces(self, face1_path: str, face2_path: str) -> float:
        """
        Compare two face crops. Uses ArcFace if deepface is available, else 128-d histograms.
        ...
        if _HAS_DEEPFACE and not self.offline_fallback:
            try:
                ...
                return float(max(0.0, 1.0 - res.get("distance", 1.0)))
            except Exception as e:
                console.log(
                    f"[yellow]DeepFace compare failed ({e}). Falling back to histogram.[/]"
                )
        ...
        if img1 is None or img2 is None:
            console.log(
                "[yellow]Warning: Could not read one or both images for face comparison.[/]"
            )
            return 0.0
...
def _print_result(
    ...
    tbl.add_row(
        "Bounding Box",
        f"x={region['x']}  y={region['y']}  " f"w={region['w']}  h={region['h']}",
    )
    tbl.add_row("Confidence", f"[green]{confidence:.4f}[/]  ({confidence*100:.2f}%)")

    desc = "biometric" if method == "arcface" else "colour histogram NOT biometric"
    tbl.add_row("Embedding Method", f"[magenta]{method}[/]")
    tbl.add_row("Embedding Dim.", f"[magenta]{emb_dim}-d[/]  ({desc})")
    console.print(
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        console.print(
            "[bold red]Usage:[/]  python modules/face_detector.py <image_path>"
        )
        sys.exit(1)
