...
def main():
    ...
    try:
        ...
        print(
            f"   [OK] deepface installed (v{getattr(deepface, '__version__', 'unknown')})"
        )
    except ImportError as e:
    ...
    try:
        ...
        result = detector.detect_and_crop(
            str(image_path), output_path="inputs/target_cropped.jpg"
        )
        ...
        if _USE_HAAR or _USE_DNN:
            ...
            print(
                "          because the retinaface dependency is decoupled in this pipeline stage."
            )
